...
from .ai_utils import formulareingaben_in_abfragesample_konvertieren, y_und_x_erstellen, preprocessing_x, \
    ai_model_als_pickle_file_ablegen, knn_pipeline, introspection_plot_und_lesehinweis_ausgeben, onehotx_und_y_erstellen,\
    sortierte_features_importance_list_erstellen, kimodelle_neu_kalibrieren_und_abspeichern, \
    sortierte_koeff_list_erstellen, vermoegensstrafrechts_urteile_codes_aufloesen, introspection_plot_und_lesehinweis_abspeichern
from .db_utils import deliktssumme_strafhoehe_png_scatterplot_erstellen, seaborn_statistik_erstellen, kategorie_scatterplot_erstellen
...
def database(request):
    ...
    vollzug_scatterplot_1000000 = DiagrammSVG.objects.get(name='vollzug_scatterplot_1000000')
    vollzug_scatterplot_200000 = DiagrammSVG.objects.get(name='vollzug_scatterplot_200000')
    hauptdelikt_scatterplot_1000000 = DiagrammSVG.objects.get(name='hauptdelikt_scatterplot_1000000')
    hauptdelikt_scatterplot_200000 = DiagrammSVG.objects.get(name='hauptdelikt_scatterplot_200000')
...
def kimodel_evaluation(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            x = preprocessing_x(x)
            scores1 = -1 * cross_val_score(loaded_model, x, y, cv=5, scoring='neg_mean_absolute_error')
            ...
            sortierte_feature_wichtigkeit_tuple_liste = sorted(zip_liste_mit_feauture_wichtigkeit_tuples,
                                                               key=lambda tup: tup[1], reverse=True)
            sortierte_feature_wichtigkeit_tuple_liste_in_prozenten = [(tup[0], tup[1] * 100) for tup in
                                                                      sortierte_feature_wichtigkeit_tuple_liste]
            ...
            zip_liste_mit_feauture_wichtigkeit_tuples_vollzugsmodel = list(
                zip(loaded_vollzugsmodel.feature_names_in_, loaded_vollzugsmodel.feature_importances_))
            sortierte_feature_wichtigkeit_tuple_liste_vollzugsmodel = sorted(zip_liste_mit_feauture_wichtigkeit_tuples_vollzugsmodel,
                                                               key=lambda tup: tup[1], reverse=True)
            sortierte_feature_wichtigkeit_tuple_liste_in_prozenten_vollzugsmodel = [(tup[0], tup[1] * 100) for tup in
                                                                      sortierte_feature_wichtigkeit_tuple_liste_vollzugsmodel]

            # introspection plot
            introspection_plot, lesehinweis = introspection_plot_und_lesehinweis_ausgeben(Urteil, xlim=200000,
                                                                                          titel="Prognose (Deliktssumme bis Fr. 200'000)",
                                                                                          cleaned_data_dict=form.cleaned_data)
            introspection_plot2, lesehinweis2 = introspection_plot_und_lesehinweis_ausgeben(Urteil, xlim=1000000,
        ...
        # kontext füllen
        context = {'val_rf_kimodel': val_rf_kimodel,
...
@login_required
def kimodel_reset(request):
    ai_model_als_pickle_file_ablegen(urteil_model=Urteil, zielwert='hoehe_strafe')
    ai_model_als_pickle_file_ablegen(urteil_model=Urteil, zielwert='vollzug')
    ai_model_als_pickle_file_ablegen(urteil_model=Urteil, zielwert='knn')
    context = {}
    return render(request, 'database/kimodel_reset.html', context)
...
def dev(request):
    ...
    all_rf_kimodel = KIModelPickleFile.objects.get(name='rf_regr_all')
    lr_kimodel = KIModelPickleFile.objects.get(name='lr_regr_all')
    linear_regression_regressor = kimodell_von_pickle_file_aus_aws_bucket_laden(filepath=lr_kimodel.file.name)
...
def datenbank_scatterplots_aktualisieren():
    kategorie_scatterplot_erstellen(Urteil,
